"""Views for triggering and managing Celery tasks."""
...
from .tasks import (
    add, multiply, hello, long_running_task,
    unreliable_task, chain_example_1, chain_example_2, chain_example_3
)
...
@require_http_methods(["GET"])
def trigger_unreliable(request):
    """Trigger the unreliable task."""
    task = unreliable_task.delay()

    return JsonResponse({
        'task_id': task.id,
        'status': 'Unreliable task triggered',
    })


@require_http_methods(["GET"])
def trigger_chain(request):
    """Trigger a chain of tasks."""
    x = int(request.GET.get('x', 5))

    # Chain: multiply by 2, add 10, then square
    task_chain = chain(
        chain_example_1.s(x),
        chain_example_2.s(),
        chain_example_3.s(),
    ...
    return JsonResponse({
        'task_id': result.id,
        'status': 'Task chain triggered',
        'initial_value': x,
        'operations': 'x * 2 + 10, then squared',
    })
